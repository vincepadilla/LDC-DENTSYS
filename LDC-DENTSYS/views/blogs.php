...
Make sure the Title and Content are on separate lines and clearly labeled.";
    ...
    // Keep only 10 recent blogs using prepared statement
    ...
    if ($total > 10) {
        ...
        $deleteStmt = $con->prepare("DELETE FROM dental_blogs ORDER BY published_at ASC LIMIT ?");
        if ($deleteStmt) {
            $deleteStmt->bind_param("i", $deleteCount);
            $deleteStmt->execute();
            $deleteStmt->close();
        }
    }
...
// Fetch blogs using prepared statement
$blogsStmt = $con->prepare("SELECT * FROM dental_blogs ORDER BY published_at DESC");
$blogsStmt->execute();
            ...
            <div class="blog-card">
                <div class="blog-card-header">
                    <span class="blog-category">
                        <i class="fas fa-tag"></i> Dental Care
                    </span>
